...
- **`lib/keap-api.js`** — the only module that talks to Keap for the report, imported *only* by the refresher. `keapFetch` does **not** retry; it raises `KeapThrottleError` on 429 and `KeapTimeoutError` on a hang, both marked `retryable`, so the refresher can distinguish "wait and try again" from "this will never work". **Every request carries a 12s `AbortSignal.timeout`.** Omitting that was a real production bug: the first cron run met a Keap connection that never answered, consumed the platform's full 300s budget, returned a 504, and stored nothing — the retry loop never even ran. A hung request must cost one attempt, not the whole job. `api/refresh-keap.js` additionally pins `maxDuration: 120` and stops starting attempts after 75s, and reports `reason: 'deadline'` (not `'throttled'`) when it stops for time — conflating those is what let the timeout hide behind a generic throttle message.
