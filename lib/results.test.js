...
describe('buildReport — refunds', () => {
  ...
  it('drops a payment plan whose collected installment was fully refunded', () => {
    // subscription: collected 82700 (installment 1). A refund of exactly that
    // installment meets the full-refund threshold → the seat is dropped and the
    // registration is not counted as a plan registration.
    const report = buildReport(
      [{ ...session({ mode: 'subscription', amount: 82700 }), refundedCents: 82700 }],
      {}
    );
    expect(report.totals.registrations).toBe(0);
    expect(report.totals.refundedCount).toBe(1);
    expect(report.totals.refundedCents).toBe(82700);
    expect(report.totals.planRegistrations).toBe(0);
  });
});
