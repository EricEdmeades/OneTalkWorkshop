...
export const PAGE_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Consent badge on a testimonial. Colour alone cannot carry the meaning —
     the label says it in words too, so a printout or a colour-blind reader
     is not left guessing which quotes are safe to use. */
  .testimonial .consent {
    display: inline-block; margin-right: 8px; padding: 1px 7px;
    border-radius: 3px; font-size: 0.66rem; font-weight: 700;
    letter-spacing: 0.06em; text-transform: uppercase; vertical-align: 1px;
    white-space: nowrap;
  }
  .consent-ok { background: #E4F2E4; color: #1F6B2B; }
  .consent-no { background: #F6E1E1; color: #99251F; }
  .consent-warn { background: #FBEBD2; color: #8A5A08; }
  .testimonial-name { color: var(--muted); font-style: italic; }
  /* Not .nps-bands: that one carries a negative top margin to tuck under a
     row of stat cards, and there are none above this line. */
  .consent-summary { margin: 4px 0 18px; color: var(--muted); font-size: 0.82rem; }

  /* A stat card that is also a link. Keeps the card's shape, gains the
     affordance — the hover shift is the only thing saying "this goes
     somewhere", so it has to be visible. */
  a.card-link { display: block; color: inherit; text-decoration: none;
    transition: border-color 150ms ease, transform 150ms ease; }
  a.card-link:hover { border-color: var(--wine); transform: translateY(-2px); }
  a.card-link .card-sub { color: var(--wine); font-weight: 600; }
  ...
  .actions-note { font-size: 0.78rem; color: var(--muted); }
  button.danger {
    padding: 10px 18px; border: 1px solid #B4342A; border-radius: 3px;
    background: #fff; color: #B4342A; font-size: 0.8rem; font-weight: 700;
    letter-spacing: 0.06em; text-transform: uppercase; cursor: pointer;
    font-family: inherit;
  }
  button.danger:hover { background: #B4342A; color: #fff; }
  .ok {
    padding: 12px 16px; margin-bottom: 24px; border-radius: 3px;
    background: #EAF4EC; border: 1px solid #4E8C5B;
    color: #2F5C39; font-size: 0.85rem;
  }
  .sr-only {
