...
// When each workshop finishes: the last day's session ends 2pm Eastern
// (EDT, UTC-4) = 18:00 UTC. Past this instant the date stops selling and
// its card comes off the page entirely.
const EVENT_END_UTC = {
  august: Date.UTC(2026, 7, 9, 18, 0, 0),
  september: Date.UTC(2026, 8, 20, 18, 0, 0),
};
...
export function isEventOver(date, now = Date.now()) {
  const end = EVENT_END_UTC[date];
  if (end == null) throw new Error(`Unknown date: ${date}`);
  return now > end;
}
...
// Exported because /results also needs to know a plan bills exactly twice, to
// value a plan registration at its full contracted amount. One definition of
// "a plan is 2 charges" — the cancel_at math and the revenue math cannot drift.
export const SUBSCRIPTION_PERIOD_COUNT = 2;
