...
const throttled = () =>
  Object.assign(new Error('Keap throttled'), { throttled: true, retryable: true });
// A hung Keap connection. This is what took the first production cron run to
// the platform's 300s ceiling, so it must be retryable AND time-bounded.
const timedOut = () =>
  Object.assign(new Error('Keap did not respond'), { timedOut: true, retryable: true });
...
describe('refreshKeapSnapshot', () => {
  ...
  it('retries a timed-out read the same way it retries a throttle', async () => {
    fetchKeapChannel.mockRejectedValueOnce(timedOut()).mockResolvedValueOnce(channel);
  ...
  it('gives up on repeated timeouts without exhausting the function budget', async () => {
    fetchKeapChannel.mockRejectedValue(timedOut());
  ...
  // The deadline is the guard that stops a slow Keap from running the function
  // into the platform timeout, which produced a 504 and stored nothing.
  it('stops starting attempts once the deadline has passed', async () => {
    fetchKeapChannel.mockImplementation(async () => {
      await new Promise((r) => setTimeout(r, 30));
      throw timedOut();
    });

    const result = await refreshKeapSnapshot({ attempts: 10, spacingMs: 0, deadlineMs: 45 });

    expect(result.reason).toBe('deadline');
    // Far fewer than the 10 it was allowed — it ran out of time first.
    expect(fetchKeapChannel.mock.calls.length).toBeLessThan(10);
  });

  it('does not let the deadline cut short a single successful attempt', async () => {
    fetchKeapChannel.mockResolvedValueOnce(channel);

    const result = await refreshKeapSnapshot({ attempts: 4, spacingMs: 0, deadlineMs: 1 });
