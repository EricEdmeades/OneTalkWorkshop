...
// browsers attach automatically — so another site could post here and ride
// the operator's cached credentials. What stops that is the hidden token
// below (see lib/admin-token.js), which an attacker cannot read.
//
// An earlier version relied on the Origin/Referer headers instead and refused
// a legitimate delete: this page sends Referrer-Policy: no-referrer, and
// Safari omits Origin on same-origin form posts, so both arrived empty.
...
export function renderSubmissionsPage(rows, { fetchedAt, deleted = 0, error = null, token = '' } = {}) {
