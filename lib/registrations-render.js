...
export function renderRegistrationsPage(
  report,
  { truncated, fetchedAt, maxSessions, unattributedRefundedCents = 0, keapError = null, keapStale = false }
) {
  ...
  const keapDownBanner = keapError
    ? `<p class="warn">Keap channel unavailable right now — showing the web (Stripe) checkout channel only. The registration roster and Keap/WooCommerce revenue are missing from these figures. Reload to retry.</p>`
    : '';
  // Serving the last good Keap data because a refresh was rate-limited. The
  // figures are real, just not to-the-minute — a quiet note, not an alarm.
  const keapStaleNote =
    !keapError && keapStale
      ? `<p class="note">Keap roster/Woo figures are cached (Keap was rate-limited on the last refresh); they'll update on the next successful read.</p>`
      : '';
